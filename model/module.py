...
def feature_differentiable_warping(
    ...
    Args:
        ...
        src_proj: [B, 3, 7] source camera projection matrix, for each source view in batch
        ref_proj: [B, 3, 7] reference camera projection matrix, for each ref view in batch
    ...
    with torch.no_grad():
        intrinsic = ref_proj[:, :, 4:7]  # [B,3,3]
        ref_R = ref_proj[:, :, 0:3]      # [B,3,3]
        src_R = src_proj[:, :, 0:3]
        ref_T = ref_proj[:, :, 3:4]      # [B,3,1]
        src_T = src_proj[:, :, 3:4]

        trans = torch.matmul(torch.inverse(src_R), (ref_T - src_T))     # [B,3,1]
        rot = torch.matmul(torch.inverse(src_R), ref_R)           # [B,3,3]
        ...
        cam_coord = (torch.inverse(intrinsic).bmm(xyz))  # [B, 3, H*W]
        cam_coords = cam_coord.unsqueeze(2).repeat(1, 1, num_depth, 1) * depth_samples.view(batch, 1, num_depth, height * width)  # [B, 3, Ndepth, H*W]
        cam_coords = cam_coords.permute(0,2,1,3)    # [B, Ndepth, 3, H*W]

        proj_cam_to_src_pixel = intrinsic.bmm(torch.cat((rot, trans), dim=2)).unsqueeze(1).repeat(1, num_depth, 1, 1)  # [B, Ndepth, 3, 4]

        proj_xyz = torch.matmul(proj_cam_to_src_pixel[:,:,:,:3], cam_coords) + proj_cam_to_src_pixel[:,:,:,-1:]  # [B, Ndepth, 3, H*W]
        proj_xyz = proj_xyz.permute(0,2,1,3)    # [B, 3, Ndepth, H*W]

        # avoid negative depth
        negative_depth_mask = proj_xyz[:, 2:] <= 1e-3
        proj_xyz[:, 0:1][negative_depth_mask] = width
        proj_xyz[:, 1:2][negative_depth_mask] = height
        proj_xyz[:, 2:3][negative_depth_mask] = 1e-3
...
def image_differentiable_warping(
    src: torch.Tensor, src_proj: torch.Tensor, ref_proj: torch.Tensor, depth_samples: torch.Tensor
):
    """Differentiable homography-based warping, implemented in Pytorch.

    Args:
        src: [B, 3, H, W] source image in batch
        ...
        depth_samples: [B, Ndepth, H, W] virtual depth layers
    Returns:
        warped_src: [B, 3, Ndepth, H, W] image on depths after perspective transformation
    """
    ...
    with torch.no_grad():
        ...
        ref_T = ref_proj[:, :, 3:4]    # [B,3,1]
        src_T = src_proj[:, :, 3:4]
        intrinsic_ref = ref_proj[:, :, 4:7]  # [B,3,3]
        intrinsic_src = src_proj[:, :, 4:7]
        ...
        xyz = torch.stack((x, y, torch.ones_like(x)))  # [3, H*W]
        xyz = torch.unsqueeze(xyz, 0).repeat(batch, 1, 1)  # [B, 3, H*W]
        proj_cam_to_src_pixel = intrinsic_src.bmm(torch.cat((rot, trans), dim=2))  # [B, 3, 4]

        cam_coord = (torch.linalg.inv(intrinsic_ref).bmm(xyz))  # [B, 3, H*W]
        cam_coord = proj_cam_to_src_pixel[:,:,:3].bmm(cam_coord)  # [B, 3, H*W]
        cam_coords = cam_coord.unsqueeze(2).repeat(1, 1, num_depth, 1) * depth_samples.view(
            batch, 1, num_depth, height * width
        )  # [B, 3, Ndepth, H*W]

        proj_xyz = cam_coords + proj_cam_to_src_pixel[:,:,3:4].view(batch, 3, 1, 1)  # [B, 3, Ndepth, H*W]
    ...
    warped_src = warped_src.view(batch, 3, num_depth, height, width)
