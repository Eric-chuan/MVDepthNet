...
class Room100Dataset(Dataset):
    ...
    def read_data(self, np_file):
        ...
        left_extrinsic, right_extrinsic = extrinsics[2], extrinsics[1]
        left_intrinsic, right_intrinsic = intrinsics[2], intrinsics[1]
        left_extrinsic[:3,:3], right_extrinsic[:3,:3] = inv(left_extrinsic[:3,:3]), inv(right_extrinsic[:3,:3])
        left_intrinsic[0, :] = left_intrinsic[0, :] * scale_x
        left_intrinsic[1, :] = left_intrinsic[1, :] * scale_y
        right_intrinsic[0, :] = right_intrinsic[0, :] * scale_x
        right_intrinsic[1, :] = right_intrinsic[1, :] * scale_y

        return left_img, right_img, depth, left_extrinsic, left_intrinsic, right_extrinsic, right_intrinsic

    def __getitem__(self, index):
        # get the sequence index
        data_path = f'{self.dataroot}/{self.meta_data[index].strip()}/data.npz'
        np_file = np.load(data_path)
        left_img, right_img, depth, \
        left_extrinsic, left_intrinsic, right_extrinsic, right_intrinsic = self.read_data(np_file)
...
if __name__ == '__main__':
    ...
    cv2.imwrite('DepthGT.png', quantilization(depth_image_cuda.numpy().transpose(1, 2, 0)))
